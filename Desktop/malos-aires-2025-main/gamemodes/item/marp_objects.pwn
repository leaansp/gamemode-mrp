...
forward QuickDropObject(playerid, playerhand, bool:running); //Igual que DropObject pero sin animacion para hotkeys mientras se corre
...
stock DropObject(playerid, playerhand)
{
	...
	if(!GetHandItem(playerid, playerhand))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"No ning�n item para tirar en esa mano.");
...
stock QuickDropObject(playerid, playerhand, bool:running)
{
	#pragma unused running
	if(GetPlayerState(playerid) != PLAYER_STATE_ONFOOT)
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Debes estar a pie!");
	if(!GetHandItem(playerid, playerhand))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"No ningun item para tirar en esa mano.");
	if(!ItemModel_HasTag(GetHandItem(playerid, playerhand), ITEM_TAG_THROW))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"No puedes tirar ese item.");
	...
	if(j == -1)
	    return SendClientMessage(playerid, COLOR_RED, "[ERROR] Se ha alcanzado el maximo de objetos permitidos, reportar a un administrador.");
...
stock LookObject(playerid)
{
	...
	if(object == -1)
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"No hay ning�n objeto cerca tuyo.");
