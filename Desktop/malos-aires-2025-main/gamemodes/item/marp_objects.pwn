...
forward QuickDropObject(playerid, playerhand, bool:running);
...
stock QuickDropObject(playerid, playerhand, bool:running)
{
	...
	if(GetPlayerState(playerid) != PLAYER_STATE_ONFOOT)
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Debes estar a pie!");
	if(!GetHandItem(playerid, playerhand))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"No hay ningun item para tirar en esa mano.");
	if(!ItemModel_HasTag(GetHandItem(playerid, playerhand), ITEM_TAG_THROW))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"No puedes tirar ese item.");
	...
	if(ItemModel_GetType(ServerObject[j][sItemID]) == ITEM_WEAPON) {
		...
		ServerFormattedLog(LOG_TYPE_ID_MONEY, .entry="/tirar_rapido", .playerid=playerid, .params=<"$%i x: %.2f y: %.2f z: %.2f", ServerObject[j][sAmount], ServerObject[j][sX], ServerObject[j][sY], ServerObject[j][sZ]>);
	}
	return 1;
}
