...
PhoneCall:PhoneCall_New(PHONE_HANDLE:from_phone, to_number, &error)
{
	...
	if(!to_phone) // No existe un teléfono creado con ese número
	{
		if(!Phone_IsValidPersonalNumber(to_number)) { // No intent? llamar a un número de persona f?sica
...
PhoneCall_OnStarted(PHONE_HANDLE:from_phone, PHONE_HANDLE:to_phone)
{
	...
	{
		PlayerActionMessage(from_playerid, 15.0, "toma su teléfono celular y marca un número.");
...
PhoneCall_OnReceived(PHONE_HANDLE:from_phone, PHONE_HANDLE:to_phone)
{
	...
	{
		PlayerDoMessage(to_playerid, 15.0, "Un teléfono est? comenzando a sonar");
...
PhoneCall_OnMessageReceived(PHONE_HANDLE:from_phone, PHONE_HANDLE:to_phone, const message[])
{
	...
	if(IsPlayerLogged(to_playerid)) {
		new str[180] = "[Voz al teléfono] dice: ";
