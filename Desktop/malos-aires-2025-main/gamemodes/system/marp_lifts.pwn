...
Lift_CreateObjects(liftid)
{
	...
	// Ascensor
    Lift[liftid][eLiftObjectID] = CreateDynamicObject(18755, Lift[liftid][eLX], Lift[liftid][eLY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][Lift[liftid][eCurrentFloor]], 0.0, 0.0, Lift[liftid][eLRZ]);

	// Puertas del ascensor abiertas
	Lift[liftid][eRDoorObjectID] = CreateDynamicObject(18756, Lift[liftid][eLX] + Lift[liftid][eRODOFFX], Lift[liftid][eLY] + Lift[liftid][eRODOFFY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][Lift[liftid][eCurrentFloor]], 0.0, 0.0, Lift[liftid][eLRZ]);
    Lift[liftid][eLDoorObjectID] = CreateDynamicObject(18757, Lift[liftid][eLX] - Lift[liftid][eRODOFFX], Lift[liftid][eLY] - Lift[liftid][eRODOFFY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][Lift[liftid][eCurrentFloor]], 0.0, 0.0, Lift[liftid][eLRZ]);
	...
	for(new i = 0; i < Lift[liftid][eTotalFloors]; i++)
	{
	    ...
	    {
			Lift_FloorDoors[liftid][i][DOOR_RIGHT] = CreateDynamicObject(18756, Lift[liftid][eFDX] + Lift[liftid][eRODOFFX], Lift[liftid][eFDY] + Lift[liftid][eRODOFFY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][i], 0.0, 0.0, Lift[liftid][eLRZ]);
        	Lift_FloorDoors[liftid][i][DOOR_LEFT] = CreateDynamicObject(18757, Lift[liftid][eFDX] - Lift[liftid][eRODOFFX], Lift[liftid][eFDY] - Lift[liftid][eRODOFFY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][i], 0.0, 0.0, Lift[liftid][eLRZ]);
	    }
	    else
	    {
		    Lift_FloorDoors[liftid][i][DOOR_RIGHT] = CreateDynamicObject(18756, Lift[liftid][eFDX], Lift[liftid][eFDY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][i], 0.0, 0.0, Lift[liftid][eLRZ]);
	        Lift_FloorDoors[liftid][i][DOOR_LEFT] = CreateDynamicObject(18757, Lift[liftid][eFDX], Lift[liftid][eFDY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][i], 0.0, 0.0, Lift[liftid][eLRZ]);
		}
		...
		format(str, sizeof(str), "%s\n(~k~~SNEAK_ABOUT~+~k~~GROUP_CONTROL_BWD~) Llamar ascensor", Lift_FloorNames[liftid][i]);
...
{
	new str[128];
	format(str, sizeof(str), "%s\n(~k~~SNEAK_ABOUT~+~k~~GROUP_CONTROL_BWD~) Usar ascensor", Lift_FloorNames[liftid][Lift[liftid][eCurrentFloor]]);
...
public Lift_MoveToFloor(liftid, floor)
{
    if(liftid >= 0 && liftid < MAX_LIFTS && floor >= 0 && floor < Lift[liftid][eTotalFloors])
	{
	    ...
	    // Movemos el ascensor
		MoveDynamicObject(Lift[liftid][eLiftObjectID], Lift[liftid][eLX], Lift[liftid][eLY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][floor], 3.0, 0.0, 0.0, Lift[liftid][eLRZ]);
		// Movemos las puertas del ascensor
		MoveDynamicObject(Lift[liftid][eRDoorObjectID], Lift[liftid][eLX], Lift[liftid][eLY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][floor], 3.0, 0.0, 0.0, Lift[liftid][eLRZ]);
		MoveDynamicObject(Lift[liftid][eLDoorObjectID], Lift[liftid][eLX], Lift[liftid][eLY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][floor], 3.0, 0.0, 0.0, Lift[liftid][eLRZ]);
...
Lift_OpenLiftDoors(liftid)
{
	if(liftid >= 0 && liftid < MAX_LIFTS)
    {
        ...
        {
			MoveDynamicObject(Lift[liftid][eRDoorObjectID], Lift[liftid][eLX] + Lift[liftid][eRODOFFX], Lift[liftid][eLY] + Lift[liftid][eRODOFFY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][Lift[liftid][eCurrentFloor]], 1.0, 0.0, 0.0, Lift[liftid][eLRZ]);
			MoveDynamicObject(Lift[liftid][eLDoorObjectID], Lift[liftid][eLX] - Lift[liftid][eRODOFFX], Lift[liftid][eLY] - Lift[liftid][eRODOFFY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][Lift[liftid][eCurrentFloor]], 1.0, 0.0, 0.0, Lift[liftid][eLRZ]);
            return 1;
		}
	}
	return 0;
...
Lift_CloseLiftDoors(liftid)
{
	if(liftid >= 0 && liftid < MAX_LIFTS)
    {
		MoveDynamicObject(Lift[liftid][eRDoorObjectID], Lift[liftid][eLX], Lift[liftid][eLY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][Lift[liftid][eCurrentFloor]], 1.0, 0.0, 0.0, Lift[liftid][eLRZ]);
		MoveDynamicObject(Lift[liftid][eLDoorObjectID], Lift[liftid][eLX], Lift[liftid][eLY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][Lift[liftid][eCurrentFloor]], 1.0, 0.0, 0.0, Lift[liftid][eLRZ]);
		return 1;
	}
	return 0;
}
...
Lift_OpenFloorDoors(liftid, floor)
{
	if(liftid >= 0 && liftid < MAX_LIFTS && floor >= 0 && floor < Lift[liftid][eTotalFloors])
	{
	    MoveDynamicObject(Lift_FloorDoors[liftid][floor][DOOR_RIGHT], Lift[liftid][eFDX] + Lift[liftid][eRODOFFX], Lift[liftid][eFDY] + Lift[liftid][eRODOFFY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][floor], 1.0, 0.0, 0.0, Lift[liftid][eLRZ]);
	    MoveDynamicObject(Lift_FloorDoors[liftid][floor][DOOR_LEFT], Lift[liftid][eFDX] - Lift[liftid][eRODOFFX], Lift[liftid][eFDY] - Lift[liftid][eRODOFFY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][floor], 1.0, 0.0, 0.0, Lift[liftid][eLRZ]);
		return 1;
	}
	return 0;
}
...
Lift_CloseFloorDoors(liftid, floor)
{
	if(liftid >= 0 && liftid < MAX_LIFTS && floor >= 0 && floor < Lift[liftid][eTotalFloors])
	{
	    MoveDynamicObject(Lift_FloorDoors[liftid][floor][DOOR_RIGHT], Lift[liftid][eFDX], Lift[liftid][eFDY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][floor], 1.0, 0.0, 0.0, Lift[liftid][eLRZ]);
	    MoveDynamicObject(Lift_FloorDoors[liftid][floor][DOOR_LEFT], Lift[liftid][eFDX], Lift[liftid][eFDY], Lift[liftid][eLZ] + Lift_FloorZOffsets[liftid][floor], 1.0, 0.0, 0.0, Lift[liftid][eLRZ]);
		return 1;
	}
	return 0;
}

static const KEY_CUSTOM_LIFT = KEY_WALK | KEY_CTRL_BACK;

hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(!IsPlayerInAnyVehicle(playerid) && KEY_PRESSED_MULTI(KEY_CUSTOM_LIFT))
    {
        for(new i = 0; i < MAX_LIFTS; i++)
        {
            for(new f = 0; f < Lift[i][eTotalFloors]; f++)
            {
                if(IsPlayerInRangeOfPoint(playerid, 0.75, Lift[i][eILX], Lift[i][eILY], Lift[i][eILZ] + Lift_FloorZOffsets[i][f]))
                {
                    ...
                    Dialog_Show(playerid, DLG_LIFT, DIALOG_STYLE_LIST, "Selecciona el piso que deseas:", string ,"Ir", "Cancelar");
                ...
                if(IsPlayerInRangeOfPoint(playerid, 0.75, Lift[i][eOLX], Lift[i][eOLY], Lift[i][eOSZ] + Lift_FloorZOffsets[i][f]))
                {
					if(Lift[i][eCurrentFloor] == f)
					    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El ascensor se encuentra actualmente en tu piso o en viaje hacia tu piso.");
					if(Lift[i][eState] == LIFT_STATE_MOVING)
						return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El ascensor se encuentra actualmente en uso, espera un momento a que se desocupe.");
				    ...
				    PlayerActionMessage(playerid, 15.0, "presiona el botón para llamar al ascensor.");
					return ~1;
				}
			}
		}
...
Dialog:DLG_LIFT(playerid, response, listitem, inputtext[])
{
	if(!response)
	    return 1;
	if(Lift[Lift_PlayerRequested[playerid]][eCurrentFloor] == listitem)
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El ascensor se encuentra actualmente en ese piso o en viaje hacia el mismo.");
	if(Lift[Lift_PlayerRequested[playerid]][eState] != LIFT_STATE_IDLE)
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El ascensor se encuentra actualmente en uso o en espera, aguarda un momento.");
