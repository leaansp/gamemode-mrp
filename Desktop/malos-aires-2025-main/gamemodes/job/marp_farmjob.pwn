...
#define JOB_FARM_MAX_PER_PAYDAY (3)
#define JOB_FARM_MAXPRODS (20)
#define JOB_FARM_PRODVALUE (65)
#define JOB_FARM_BONUS_PER_WORKED_TIMES (1 / JOB_FARM_MAX_PER_PAYDAY)
#define JOB_FARM_BONUS_PER_RANK (100 / JOB_FARM_MAX_PER_PAYDAY)

#define JOB_FARM_TRAVEL_TIME    10 // En minutos
...
FarmJob_StartWork(playerid)
{
	...
	if(Veh_GetSystemType(vehicleid) != VEH_JOB || Veh_GetJob(vehicleid) != JOB_FARM)
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"¡Debes ingresar a una cosechadora!");
	if(PlayerInfo[playerid][pCantWork] >= JOB_FARM_MAX_PER_PAYDAY)
...
FarmJob_FinishWork(playerid)
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 20.0, JOB_FARM_POS[17][0], JOB_FARM_POS[17][1], JOB_FARM_POS[17][2]))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"¡Ve y descarga los productos en la zona indicada!");
	if(Veh_GetJob(GetPlayerVehicleID(playerid)) != JOB_FARM)
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"¡Debes estar adentro de una cosechadora!");

	PlayerActionMessage(playerid, 15.0, "descarga todo el producto y estaciona la cosechadora.");
	SendFMessage(playerid, COLOR_INFO, "[INFO] "COLOR_EMB_GREY"Has recolectado %d toneladas por lo tanto recibirás $%d en el próximo día de pago menos los descuentos por roturas.", CollectedProds[playerid], CollectedProds[playerid] * JOB_FARM_PRODVALUE);
...
FarmJob_Payment(playerid, vehicleid)
{
	...
	seniorityBonus = PlayerJobInfo[playerid][pWorkingHours] * JOB_FARM_BONUS_PER_WORKED_TIMES; // Extra por antiguedad
	chargeBonus = (PlayerJobInfo[playerid][pCharge] - 1) * JOB_FARM_BONUS_PER_RANK; // Extra por cargo
...
hook function OnPlayerEnterCPId(playerid, checkpointid)
{
	...
	if(!FarmJob_IsPlayerWorking(playerid, GetPlayerVehicleID(playerid)))
		return continue(playerid, checkpointid);
