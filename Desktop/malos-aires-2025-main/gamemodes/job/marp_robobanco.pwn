 #if defined marp_robobanco_inc
	#endinput
#endif
#define marp_robobanco_inc
...
stock PrintRobberyGroupInfo(playerid)
{
	...
	SendFMessage(playerid, COLOR_WHITE, " Jefe: %s (ID %d)", GetPlayerCleanName(RobberyGroup[LeaderID]), RobberyGroup[LeaderID]);
	for(new i = 0; i < GROUP_MEMBERS; i++)
	{
	    ...
	    if(memberid != INVALID_PLAYER_ID)
	    {
	        if(IsPlayerConnected(memberid))
 				SendFMessage(playerid, COLOR_WHITE, " %d - Miembro: %s (ID %d)" , i, GetPlayerCleanName(memberid), memberid);
			else
			    SendFMessage(playerid, COLOR_WHITE, " %d - Miembro: %s (Desconectado)" , i, GetPlayerCleanName(memberid));
		}
		else
...
public OnPlayerLeaveRobberyGroup(playerid, reason)
{
	...
	if(isPlayerInRobberyGroup[playerid])
	{
		...
		{
	 		if(reason == 1)
	 		    format(str, sizeof(str), "El grupo de robo ha sido eiminado ya que el lider %s se ha desconectado.", GetPlayerCleanName(playerid));
	 		else if(reason == 2)
	 		    format(str, sizeof(str), "El grupo de robo ha sido eiminado ya que el lider %s ha muerto.", GetPlayerCleanName(playerid));
		...
		} else
			{
				...
				{
					if(reason == 1)
	                    format(str, sizeof(str), "[Grupo de robo]: %s se ha desconectado pero es posible continuar sin él.", GetPlayerCleanName(playerid));
					else if(reason == 2)
		 		    	format(str, sizeof(str), "[Grupo de robo]: %s ha muerto pero es posible continuar sin él.", GetPlayerCleanName(playerid));
                    RobberyGroupMessage(COLOR_WHITE, str);
					RemoveRobberyGroupMember(playerid);
				} else
				    {
		                if(reason == 1)
		                    format(str, sizeof(str), "[Grupo de robo]: El grupo ha sido eliminado ya que %s se ha desconectado y no es posible continuar sin él.", GetPlayerCleanName(playerid));
						else if(reason == 2)
			 		    	format(str, sizeof(str), "[Grupo de robo]: El grupo ha sido eliminado ya que %s ha muerto y no es posible continuar sin él.", GetPlayerCleanName(playerid));
                        RobberyGroupMessage(COLOR_WHITE, str);
				  		ResetRobberyGroup();
					}
			}
...
{
	if(!JobThief_LevelCheck(playerid, .felonLevel = 6))
		return 1;
	if(isPlayerInRobberyGroup[playerid])
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Ya te encuentras en un grupo de robo.");
	if(!CreateRobberyGroup(playerid))
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Ya hay otro grupo de robo al banco en curso.");

	SendClientMessage(playerid, COLOR_WHITE, "Has creado un grupo de robo para asaltar el Banco Central de Malos Aires. Más info en /grupoayuda.");
  	return 1;
}
...
CMD:grupoinvitar(playerid, params[])
{
	...
	if(sscanf(params, "u", targetid))
	   	return SendClientMessage(playerid, COLOR_USAGE, "[USO] "COLOR_EMB_GREY"/grupoinvitar  [ID/Jugador]");
	...
	if(PlayerInfo[targetid][pJob] != JOB_FELON)
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El sujeto tiene que tener el trabajo de delincuente.");
	if(ThiefJobInfo[targetid][pFelonLevel] < 4)
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El sujeto necesita más experiencia como delincuente para formar un grupo.");
	if(isPlayerInRobberyGroup[targetid])
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Este sujeto ya pertenece a un grupo de robo.");
	if(pRobberyGroupOffer[targetid] != INVALID_PLAYER_ID)
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El sujeto tiene una invitación pendiente, aguarda.");

 	pRobberyGroupOffer[targetid] = playerid;
 	SendFMessage(playerid, COLOR_LIGHTBLUE, "Invitaste a %s a formar parte de tu grupo de robo. La oferta termina en 10 segs.", GetPlayerCleanName(targetid));
 	SendFMessage(targetid, COLOR_LIGHTBLUE, "%s te invitó a su grupo de robo. Para aceptar usa /grupoaceptar. La oferta termina en 10 segs.", GetPlayerCleanName(playerid));
...
CMD:grupoaceptar(playerid, params[])
{
	...
	if(leaderid == INVALID_PLAYER_ID)
        return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Nadie te está invitando a un grupo.");
	if(isPlayerInRobberyGroup[playerid])
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Ya perteneces a un grupo de robo.");
    if(!IsPlayerConnected(leaderid) || !IsPlayerRobberyGroupLeader(leaderid))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Jugador inválido / El sujeto no es lider de ningún grupo.");
	if(!IsPlayerInRangeOfPlayer(4.0, playerid, leaderid))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El sujeto se encuentra muy lejos.");
	if(!AddRobberyGroupMember(playerid))
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"¡No hay mas espacio para miembros en ese grupo!");

	format(str, sizeof(str), "%s ha ingresado al grupo de robo al banco. Mas información en /grupoayuda.", GetPlayerCleanName(playerid));
...
CMD:grupoexpulsar(playerid, params[])
{
	...
	if(!IsPlayerRobberyGroupLeader(playerid))
    	return 1;
	if(sscanf(params, "u", targetid))
		return SendClientMessage(playerid, COLOR_USAGE, "[USO] "COLOR_EMB_GREY"/grupoexpulsar  [ID/Jugador]");
  	if(!IsPlayerConnected(targetid) || targetid == playerid)
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Jugador inválido. Revisa el estado de tu grupo con /grupoinfo.");
	if(!RemoveRobberyGroupMember(targetid))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Jugador inválido. Revisa el estado de tu grupo con /grupoinfo.");

	format(str, sizeof(str), "El lider %s ha expulsado a %s del grupo de robo al banco.", GetPlayerCleanName(playerid), GetPlayerCleanName(targetid));
	RobberyGroupMessage(COLOR_WHITE, str);
	return 1;
}
...
CMD:grupoabandonar(playerid, params[])
{
	...
	if(IsPlayerRobberyGroupLeader(playerid))
    	return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"No puedes usar este comando siendo el líder. Puedes eliminar el grupo con /grupoeliminar.");
	if(isPlayerInRobberyGroup[playerid])
	{
	    format(str, sizeof(str), "%s ha abandonado el grupo de robo al banco.", GetPlayerCleanName(playerid));
	    RobberyGroupMessage(COLOR_WHITE, str);
	    RemoveRobberyGroupMember(playerid);
	}
	return 1;
...
{
	if(!IsPlayerRobberyGroupLeader(playerid))
    	return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"No eres líder de ningún grupo de robo.");
...
CMD:robarbanco(playerid, params[])
{
	...
	if(!IsPlayerRobberyGroupLeader(playerid))
    	return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Debes ser el líder de un grupo de robo para utilizar este comando.");
	if(bankRobberyCooldown > 0)
	    return SendClientMessage(playerid, COLOR_INFO, "[INFO] "COLOR_EMB_GREY"Las bóvedas del banco están vacias, no tiene sentido. (OOC: El banco todavía no está disponible para asaltar)");
	if(!IsPlayerInRangeOfPoint(playerid, 5.0, POS_BANK_MAIN_DOOR_X, POS_BANK_MAIN_DOOR_Y, POS_BANK_MAIN_DOOR_Z))
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"El grupo completo debe estar en el interior del Banco Central de Malos Aires, cercanos a la puerta principal.");
	if(!hasFireGun(playerid))
		return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Debes tener un arma de fuego en la mano para iniciar el asalto.");
	for(new i = 0; i < GROUP_MEMBERS; i++)
 	{
 	    memberid = RobberyGroup[MembersID][i];
		if(memberid != INVALID_PLAYER_ID && IsPlayerConnected(memberid))
		{
			...
			if(!hasFireGun(memberid))
				return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Todo tu equipo debe contar con algún arma de fuego en la mano.");
		}
	}
 	if(CountCopsOnDuty() < MIN_DUTYCOPS_NEEDED)
 	{
 		SendFMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Deben haber mínimo %d policias en servicio conectados.", MIN_DUTYCOPS_NEEDED);
 		return 1;
	}
	if(CountRobberyGroupMembers() < MIN_THIEFS_NEEDED)
	{
		SendFMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Recuerda que en tu grupo de robo deben ser %d, de lo contrario no podras robar el banco.", MIN_THIEFS_NEEDED);
		return 1;
	}
	if(GetRobberyGroupStage() != STAGE_INICIO_ROBO)
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Tu grupo ya se encuentra en una etapa mas avanzada del robo.");
    ...
    format(string, sizeof(string), "[ADMIN INFO]: %s (ID %d) ha iniciado un robo al Banco Central de Malos Aires.", GetPlayerCleanName(playerid), playerid);
	...
	format(string, sizeof(string), "[Dpto. de policía]: Ha sonado la alarma del Banco Central de Malos Aires. ¡Lo están robando!");
 	foreach(new i : Player)
 	{
		if(isPlayerCopOnDuty(i))
		{
			...
			SendClientMessage(i, COLOR_CENTRALRED, string);
			SendClientMessage(i, COLOR_PMA, "================================[URGENTE]==================================");
		}
 	}
}
...
CMD:reducirempleados(playerid, params[])
{
	...
	if(GetRobberyGroupStage() != STAGE_REDUCIR_ENTORNO)
	    return SendClientMessage(playerid,COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Tu grupo se encuentra en otra etapa del robo.");
	if(!hasFireGun(playerid))
...
CMD:detonarpuerta(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 1.4, POS_BANK_ACCESS_DOOR_X, POS_BANK_ACCESS_DOOR_Y, POS_BANK_ACCESS_DOOR_Z))
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Debes estar junto a la puerta de acceso al resto del edificio.");
...
CMD:detonarboveda(playerid, params[])
{
	...
	if(GetRobberyGroupStage() != STAGE_DETONAR_BOVEDA)
 	...
 	if(!IsPlayerInRangeOfPoint(playerid, 1.4, POS_BANK_SAFE_DOOR_X, POS_BANK_SAFE_DOOR_Y, POS_BANK_SAFE_DOOR_Z))
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Debes estar junto a la bóveda del banco.");
...
public DetonarBoveda(playerid)
{
	...
	if(isPlayerInRobberyGroup[playerid])
	{
		CreateExplosion(POS_BANK_SAFE_DOOR_X, POS_BANK_SAFE_DOOR_Y, POS_BANK_SAFE_DOOR_Z, 0, 8.0);
		RobberyGroup[RobberyStageProgress]++;
		if(RobberyGroup[RobberyStageProgress] == STAGE_COMPLETED)
		{
...
{	
	...
	if(GetRobberyGroupStage() != STAGE_TOMAR_DINERO)
   	...
   	if(!IsPlayerInRangeOfPoint(playerid, 4.0, POS_BANK_INSIDE_SAFE_X, POS_BANK_INSIDE_SAFE_Y, POS_BANK_INSIDE_SAFE_Z))
	    return SendClientMessage(playerid, COLOR_ERROR, "[ERROR] "COLOR_EMB_GREY"Debes estar dentro de la bóveda del Banco.");
	...
	TogglePlayerControllable(playerid, false);
	PlayerInfo[playerid][pDisabled] = DISABLE_STEALING; // Para no descongelarse con /noanim.
