...
// Util para normalizar un ángulo o resultados de cuentas entre ángulos al rango (0-360)Â°.
...
{
	new Float:min_range = NormalizeAngle(original_angle - tolerance), // Si por ejemplo quedase -20Â°, normalizandolo al intervalo positivo 0-360 nos quedaría 340Â°.
	    Float:max_range = NormalizeAngle(original_angle + tolerance); // Si por ejemplo quedase 380Â°, normalizandolo al intervalo 0-360 nos quedaría 20Â°.
	    
	if(max_range - min_range < 0.0) // Es un rango que me da la vuelta por el 0Â° / 360Â°. Ej: > 320Â° y < 40Â°, donde min_range se invirtió de -40Â° a 320Â°, y max_range es 40Â°
	    return (check_angle >= min_range || check_angle <= max_range);
	else // No da la vuelta. Ej: > 100Â° y < 180Â°, donde min_range es 100Â° y max_range 180Â°
	    return (check_angle >= min_range && check_angle <= max_range);
}
