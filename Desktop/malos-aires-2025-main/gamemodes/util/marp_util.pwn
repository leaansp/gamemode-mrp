...
new Mobile[MAX_PLAYERS],
	...
	bool:AdminInvisible[MAX_PLAYERS];
...
IsValidSkin(skin) {
	return (((1 <= skin <= 311) && skin != 74) || (20001 <= skin <= 20020));
}
...
SendPlayerMessageInRange(Float:range, playerid, const string[], col1, col2, col3, col4, col5, show_self = 1)
{
	...
	foreach(new i : Player)
	{
		if(GetPlayerVirtualWorld(i) == vworld)
		{
			...
			else if(Specteate_GetSpectatingVehID(i) == GetPlayerVehicleID(playerid)) {
				SendClientMessage(i, col5, string);
			}
		}
	}
...
stock Util_CapitalizeText(const input[], output[], len = sizeof output) 
{
	for(new i = 0; i < len; i++)
	{
		if(input[i]) {
